...
cout << "Для ЦИВ\n";
	...
	virtual void calculateP() {
		...
		P1t = pow(Eat1 * tmp, 2);
cout << "Для ТФ\n";
		cout << " 1) P = (" << Eat1 << " * " << tmp << ")^2 = " << P1t << " кВт\n";
		P2t = pow(Eat2 * tmp, 2);
		cout << " 2) P = (" << Eat2 << " * " << tmp << ")^2 = " << P2t << " кВт\n";
cout << "Для УБПЧ\n";
		P1u = pow(Eau1 * tmp, 2);
		cout << " 1) P = (" << Eau1 << " * " << tmp << ")^2 = " << P1u << " кВт\n";
		P2u = pow(Eau2 * tmp, 2);
		cout << " 2) P = (" << Eau2 << " * " << tmp << ")^2 = " << P2u << " кВт\n";
cout << "Для ЦИВ\n";
		P1c = pow(Eac1 * tmp, 2);
		cout << " 1) P = (" << Eac1 << " * " << tmp << ")^2 = " << P1c << " кВт\n";
		P2c = pow(Eac2 * tmp, 2);
		cout << " 2) P = (" << Eac2 << " * " << tmp << ")^2 = " << P2c << " кВт\n";
	}
